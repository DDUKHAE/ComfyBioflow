...
def test_macs3_peak_calling_runs_one_command_per_sample(tmp_path):
    # Regression coverage: Macs3PeakCallingNode now runs a samtools paired-end detection
    # command before macs3 callpeak, instead of always assuming -f BAMPE.
    runner = DryRunCommandRunner()
    ...
    assert result == (str(out),)
    assert len(runner.commands) == 2
    assert runner.commands[0].argv[:6] == ["conda", "run", "-n", "epigenomics", "samtools", "view"]
    assert runner.commands[1].argv[:5] == ["conda", "run", "-n", "epigenomics", "macs3"]
    assert (out / "sample_a").exists()


def test_macs3_peak_calling_defaults_to_bampe_when_detection_is_inconclusive(tmp_path):
    # DryRunCommandRunner returns empty stdout for the detection command (no real samtools
    # execution), which must default to BAMPE rather than crash on int("").
    runner = DryRunCommandRunner()
    input_dir = _filtered_fixture(tmp_path)
    node = Macs3PeakCallingNode()
    node.run(
        filtered_bam_dir="upstream", input_dir=str(input_dir), output_dir=str(tmp_path / "peaks"),
        genome_size="hs", extra_command="", runner=runner,
    )
    macs3_argv = runner.commands[1].argv
    assert "-f" in macs3_argv and "BAMPE" in macs3_argv


class _StubRunner:
    """Returns a fixed samtools paired-read count, then records macs3 argv like DryRunCommandRunner."""

    def __init__(self, paired_read_count: int):
        self._paired_read_count = paired_read_count
        self.commands = []

    def run(self, argv, cwd):
        from bioflow_harness.runtime.command_runner import CommandRecord

        if "view" in argv and "-c" in argv:
            record = CommandRecord(argv=argv, cwd=cwd, dry_run=False, returncode=0, stdout=str(self._paired_read_count))
        else:
...
def test_macs3_peak_calling_uses_bam_format_for_single_end_bam(tmp_path):
    runner = _StubRunner(paired_read_count=0)
    input_dir = _filtered_fixture(tmp_path)
    node = Macs3PeakCallingNode()
    node.run(
        filtered_bam_dir="upstream", input_dir=str(input_dir), output_dir=str(tmp_path / "peaks"),
    ...
    macs3_argv = runner.commands[1].argv
    assert "-f" in macs3_argv
    assert macs3_argv[macs3_argv.index("-f") + 1] == "BAM"


def test_macs3_peak_calling_uses_bampe_format_for_paired_end_bam(tmp_path):
    runner = _StubRunner(paired_read_count=42)
    input_dir = _filtered_fixture(tmp_path)
    node = Macs3PeakCallingNode()
    node.run(
        filtered_bam_dir="upstream", input_dir=str(input_dir), output_dir=str(tmp_path / "peaks"),
    ...
    macs3_argv = runner.commands[1].argv
    assert "-f" in macs3_argv
    assert macs3_argv[macs3_argv.index("-f") + 1] == "BAMPE"
