...
def test_macs3_callpeak_argv_defaults_to_bampe_and_genome_size():
...
def test_macs3_callpeak_argv_accepts_explicit_format_flag_for_single_end():
    argv = macs3_callpeak_argv("/tmp/final.bam", "/tmp/peaks/sample_a", "sample_a", "hs", format_flag="BAM")
    assert "-f" in argv
    assert argv[argv.index("-f") + 1] == "BAM"


def test_samtools_count_paired_argv_checks_flag_0x1():
    argv = samtools_count_paired_argv("/tmp/final.bam")
    assert argv[:6] == ["conda", "run", "-n", "epigenomics", "samtools", "view"]
    assert "-c" in argv
    assert "-f" in argv and "1" in argv
    assert "/tmp/final.bam" in argv
